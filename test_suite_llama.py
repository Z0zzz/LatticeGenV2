...
if model_path == "./tmp/2gram_n2_model/step_2300":
    ...
    ngram = 2
elif "1gram_n2_model" in model_path:
...
for mix_ratio in mix_ratios:
    ...
    for repetition_ratio in repetition_ratios:
        ...
        for i in range(size):
            ...
            # allow_repeat_positionss = allow_repeat_positions + [prompt_len - 2, prompt_len - 1]
            ...
            for jj in range(1):
                ...
                print("#"*20)
                del model.base_model.model.past_key_values
                del text
                del model.base_model.model.noised_history
                del model.base_model.model.sequence_idx_map # entry 0 is true sequence, the rest are noise sequences # random.choices(list( (ngram)), k=ngram)   # randonly setting bos tokens to be the true token
                del model.base_model.model.true_sequence
                del model.base_model.model.noise_sequences
                del model.base_model.model.timestep_logits   # dictionary of dictionaries, outer key is the integer from 0 to total_length - 1, indicating the timestep of taking the logit, inner key are the ngrams
