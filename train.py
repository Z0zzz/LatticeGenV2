...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "use continuous(e.g., [-1,-2,-3,-4]) or long history(e.g., [-1,-2,-4,-8])",
            "if passed, use long history"
        ),
    )
    parser.add_argument(
        "--parallel_data",
...
def main():
    ...
    train_dataset = WritingPromptsDataset(tokenizer, args.block_size, "train",size=10000)
    eval_dataset = WritingPromptsDataset(tokenizer, args.block_size, "valid", size=500)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                if args.use_long_history:
                    new_batch = model.generate_training_batch_extended_history(batch)
                elif args.parallel_data:
            ...
            if (step+1) % 200 == 0:
                ...
                try:
                    ...
                    print("checkpoint eval loss: ", eval_loss)
