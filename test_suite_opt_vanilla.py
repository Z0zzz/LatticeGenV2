...
model_path = "vanilla-opt/step_2000"
ngram = 4
n_noise_tokens = 2
noise_sample_topk = 5
num_beams = 50
...
if "vanilla" in model_path:
    model = OPTForCausalLM.from_pretrained("./opt-models/vanilla-opt").cuda()

elif model_path == "./tmp/2gram_n2_model/step_2300":
    ...
    ngram = 4
    model = OPTForCausalLM.from_pretrained(model_path).cuda()
    ...
    for repetition_ratio in repetition_ratios:
        ...
        for i in range(size):
            ...
            for jj in range(1):
                ...
                print(decoded_text)
                print(f"{ngram} gram n = {n_noise_tokens} avg generation slow down: ", sum(model.times)/len(model.times))
